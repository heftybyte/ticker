...
const proxy_coinmarketcap = require('./coinmarketcap');
...
const getPricesFromCoinMarketCap = async (tsym) => {
  let err;
  const prices = {};
  const tsymLower = tsym.toLowerCase()
  let currencies = await proxy_coinmarketcap.ticker({
    limit: 0,
    convert: tsym,
  }).catch(e=>err = e);
  ...
  currencies.forEach((currency) => {
    prices[currency.symbol] = {
      ...
      price: Number(currency[`price_${tsymLower}`]) || 0,
      change: Number(currency['percent_change_24h']) || 0,
      change7d: Number(currency['percent_change_7d']) || 0,
      period: '24h',
      marketCap: Number(currency[`market_cap_${tsymLower}`]) || 0,
      volume24Hr: Number(currency[`24h_volume_${tsymLower}`]) || 0,
...
// getPricesFromCoinMarketCap("USD")
...
export const getPricesForSymbol = async (tsym) => {
  ...
  const prices = symbols.map(fsym => {
    const priceData = cmcPrices[fsym] || ccPrices[fsym] || {};
    return {
      fsym,
      tsym,
      ...
      supply: Number.MAX_SAFE_INTEGER < priceData.supply ? 0 : parseInt(priceData.supply)
    }
  });
